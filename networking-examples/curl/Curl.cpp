...
Request::Request(Curl *owner, const char *url)
    : owner_(owner), curl_(curl_easy_init()) {
...
int Curl::socketCallback(CURL *c, int fd, int what, void *userp,
  ...
  if (what == CURL_POLL_REMOVE) {
    networking::Channel *ch = static_cast<Channel *>(socketp);
...
int Curl::timerCallback(CURLM *curlm, long ms, void *userp) {
  ...
  curl->loop_->runAfter(static_cast<int>(ms) / 1000.0,
                        std::bind(&Curl::onTimer, curl));
  return 0;
}

Curl::Curl(EventLoop *loop)
    : loop_(loop), curlm_(CHECK_NOTNULL(curl_multi_init())), runningHandles_(0),
