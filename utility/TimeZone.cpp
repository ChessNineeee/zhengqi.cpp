...
struct TimeZone::Data {
    struct Transition {
        ...
        Transition(int64_t u, int64_t l, int localIdx)
                : utctime(u), localtime(l), localtimeIdx(localIdx) {}
    };
...
// RFC 8536: https://www.rfc-editor.org/rfc/rfc8536.html
        bool readDataBlock(File &f, struct TimeZone::Data *data, bool v1) {
            ...
            data->localtimes_.reserve(typecnt);
...
TimeZone::Data::findLocalTime(int64_t utcTime) const {
    ...
    if (transitions_.empty() || utcTime < transitions_.front().utctime) {
        // FIXME: should be first non dst time zone
        local = &localtimes_.front();
    } else {
        Transition sentry(utcTime, 0, 0);
        std::vector<Transition>::const_iterator transI = std::upper_bound(
                transitions_.begin(), transitions_.end(), sentry, CompareUtcTime());
        assert(transI != transitions_.begin());
        if (transI != transitions_.end()) {
            --transI;
            local = &localtimes_[transI->localtimeIdx];
        } else {
            // FIXME: use TZ-env
            local = &localtimes_[transitions_.back().localtimeIdx];
        }
    }
...
TimeZone::Data::findLocalTime(const struct DateTime &lt,
    ...
    if (transitions_.empty() || localtime < transitions_.front().localtime) {
        // FIXME: should be first non dst time zone
        return &localtimes_.front();
    }

    Transition sentry(0, localtime, 0);
    std::vector<Transition>::const_iterator transI = std::upper_bound(
            transitions_.begin(), transitions_.end(), sentry, CompareLocalTime());
    assert(transI != transitions_.begin());

    if (transI == transitions_.end()) {
        // FIXME: use TZ-env
        return &localtimes_[transitions_.back().localtimeIdx];
    }

    Transition prior_trans = *(transI - 1);
    int64_t prior_second =
            transI->utctime - 1 + localtimes_[prior_trans.localtimeIdx].utcOffset;

    // row UTC time             isdst  offset  Local time (PRC)     Prior second
    // local time
    //  1  1989-09-16 17:00:00Z   0      8.0   1989-09-17 01:00:00
    //  2  1990-04-14 18:00:00Z   1      9.0   1990-04-15 03:00:00  1990-04-15
    //  01:59:59 3  1990-09-15 17:00:00Z   0      8.0   1990-09-16 01:00:00
    //  1990-09-16 01:59:59 4  1991-04-13 18:00:00Z   1      9.0   1991-04-14
    //  03:00:00  1991-04-14 01:59:59 5  1991-09-14 17:00:00Z   0      8.0
    //  1991-09-15 01:00:00

    // input 1991-04-14 02:30:00, found row 4,
    //  4  1991-04-13 18:00:00Z   1      9.0   1991-04-14 03:00:00  1991-04-14
    //  01:59:59
    if (prior_second < localtime) {
        ...
        // transI->localtime);
        if (postTransition) {
            return &localtimes_[transI->localtimeIdx];
        } else {
            return &localtimes_[prior_trans.localtimeIdx];
        }
    }

    // input 1990-09-16 01:30:00, found row 4, looking at row 3
    //  3  1990-09-15 17:00:00Z   0      8.0   1990-09-16 01:00:00  1990-09-16
    //  01:59:59
    --transI;
    if (transI != transitions_.begin()) {
...
int64_t TimeZone::fromLocalTime(const struct DateTime &localtime,
                                bool postTransition) const {
    assert(data_ != NULL);
