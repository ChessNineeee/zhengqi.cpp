...
std::string uptime(Timestamp now, Timestamp start, bool showMicroseconds) {
  ...
  if (showMicroseconds) {
    int microseconds =
        static_cast<int>(age % Timestamp::kMicroSecondsPerSecond);
    snprintf(buf, sizeof buf, "%d days %02d:%02d:%02d.%06d", days, hours,
             minutes, seconds % 60, microseconds);
  } else {

    snprintf(buf, sizeof buf, "%d days %02d:%02d:%02d", days, hours, minutes,
             seconds % 60);
  }
...
StringPiece next(StringPiece data) {
  const char *sp = static_cast<const char *>(
      ::memchr(data.data(), ' ', static_cast<size_t>(data.size())));
...
std::string ProcessInspector::overview(HttpRequest::Method,
  ...
  result += " (UTC), up for ";
  result += uptime(now, ProcessInfo::startTime(), true);
  result += "\n";
  ...
  stringPrintf(
      &result, "%.3f MiB, ",
      static_cast<double>(static_cast<double>(getLong(procStatus, "VmSize:")) /
                          1024.0));
...
std::string ProcessInspector::pid(HttpRequest::Method,
                                  const Inspector::ArgList &) {
  char buf[32];
...
std::string ProcessInspector::procStatus(HttpRequest::Method,
                                         const Inspector::ArgList &) {
  return ProcessInfo::procStatus();
}

std::string ProcessInspector::openedFiles(HttpRequest::Method,
                                          const Inspector::ArgList &) {
  char buf[32];
...
std::string ProcessInspector::threads(HttpRequest::Method,
                                      const Inspector::ArgList &) {
    ...
    if (FileUtil::readFile(buf, 65536, &stat) == 0) {
      ...
      *const_cast<char *>(rp) = '\0'; // don't do this at home
      ...
      snprintf(buf, sizeof buf, "%5d %-16s %c %12.3f %12.3f\n", tid,
               name.data(), *state, t.userSeconds, t.systemSeconds);
      result += buf;
    }
  }
  return result;
